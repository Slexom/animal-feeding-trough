...
public class SelfFeedAxolotl extends SelfFeed {
	...
	protected boolean canStillUse(ServerLevel level, Animal animal, long timestamp) {
		...
		boolean isWaterlogged = level.getBlockState(blockPos).getValue(BlockStateProperties.WATERLOGGED);

		return isWaterlogged && animal.canFallInLove() && animal.getAge() == 0 && !animal.getBrain().hasMemoryValue(MemoryModuleType.BREED_TARGET) && !animal.getBrain().hasMemoryValue(MemoryModuleType.IS_PANICKING);
	}
