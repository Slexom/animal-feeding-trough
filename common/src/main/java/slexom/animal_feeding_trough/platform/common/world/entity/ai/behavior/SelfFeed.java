...
public class SelfFeed extends Behavior<Animal> {
	...
	public static final double DEFAULT_CLOSE_ENOUGH_DIST = 2.0;
...
//		return animal.getBrain().hasMemoryValue(
...
//	}
	...
	protected boolean canStillUse(ServerLevel level, Animal animal, long timestamp) {
		return this.getFeedingTrough(animal).isPresent() && animal.canFallInLove() && animal.getAge() == 0 && !animal.getBrain().hasMemoryValue(MemoryModuleType.BREED_TARGET) && !animal.getBrain().hasMemoryValue(MemoryModuleType.IS_PANICKING);
	}

	protected void start(ServerLevel level, Animal animal, long timestamp) {
		animal.getBrain().setMemory(MemoryModuleType.IS_TEMPTED, true);
	}

	protected void stop(ServerLevel level, Animal animal, long timestamp) {
	...
	protected void tick(ServerLevel level, Animal animal, long timestamp) {
		Optional<FeedingTroughBlockEntity> trough = this.getFeedingTrough(animal);

		if (trough.isEmpty()) {
			return;
		}

		FeedingTroughBlockEntity feedingTroughBlockEntity = trough.get();
